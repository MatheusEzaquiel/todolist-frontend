...
const updateOrderTasks = async (listId, taskIds) => {
  try {
    console.log(listId + " - " + taskIds)
    const { data } = await Api().put(`/tasks/reorder`, 
        {checklistId: listId, taskListId: taskIds,});
    return data;
  } catch (ex) {
    throw new ApiException(
      ex.response?.data?.error || "Error updating task order"
    );
  }
};
